...
void dimmer_setup(void) {
    uc_timer_pwm_pin_setup(PWM_PIN_PORT_CLOCK, \
    PWM_PIN_PORT, PWM_PIN);
    uc_timer_pwm_setup(PWM_TIMER_CLOCK, PWM_TIMER, PWM_TIMER_CHANNEL, \
    PWM_PRESCALER);
    uc_timer_config_period(PWM_TIMER, PWM_PERIOD);
    // uc_timer_pwm_config_duty_cycle(PWM_TIMER, PWM_TIMER_CHANNEL,
    // get_pwm_percentage_counts(dimPercentage));
...
void dimmer_update_percentage(uint16_t percentage) {
    uc_timer_pwm_config_duty_cycle(PWM_TIMER, PWM_TIMER_CHANNEL, \
    get_pwm_percentage_counts(percentage));
}
